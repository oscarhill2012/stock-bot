"""Unit tests for ``src/config/retry_transport.py`` — Pydantic-validated
loader for ``config/retry_transport.json``.
...
``_reset_cache()``.  Mirrors ``test_retry_429_config.py`` — each test points
the loader at a temporary JSON file so the real
``config/retry_transport.json`` is never mutated.
"""
...
from config.retry_transport import (
    RetryTransportPolicy,
    _reset_cache,
    load_retry_transport_policy,
)
...
def test_load_retry_transport_policy_valid_payload(tmp_path) -> None:
    """A valid payload loads into a ``RetryTransportPolicy`` with the
    expected values.
    ...
    cfg_file = tmp_path / "retry_transport.json"
    payload = {**_MINIMAL_CFG, "_comment": "should be stripped"}
    cfg_file.write_text(json.dumps(payload))

    cfg = load_retry_transport_policy(path=cfg_file)

    assert isinstance(cfg, RetryTransportPolicy)
    assert cfg.max_attempts       == 4
    assert cfg.base_delay_seconds == 2.0
    assert cfg.max_delay_seconds  == 30.0


def test_load_retry_transport_policy_rejects_invalid_field(tmp_path) -> None:
    ...
    with pytest.raises(ValidationError):
        load_retry_transport_policy(path=cfg_file)


def test_load_retry_transport_policy_rejects_inverted_delay_bounds(tmp_path) -> None:
...
def test_reset_cache_cycles_lru_cache(tmp_path) -> None:
    ...
    ``lru_cache`` on ``get_retry_transport_policy`` was actually cleared.
    """

    cfg_file = tmp_path / "retry_transport.json"
    cfg_file.write_text(json.dumps(_MINIMAL_CFG))

    first = load_retry_transport_policy(path=cfg_file)

    # Reset and reload — the result must be a fresh object, not the same
    # instance that was constructed on the first call.
    _reset_cache()
    second = load_retry_transport_policy(path=cfg_file)
