...
def test_fundamental_branch_wires_llm_caps_from_config() -> None:
    ...
    passes them through correctly to the LlmAgent and RetryingAgentWrapper."""
    ...
    # ``AnalystHeuristics`` stores the fundamental vocab under
    # ``fundamental_vocabulary`` (not ``.fundamental``), so we use the
    # correct field name — mirrors the pattern used in the News test above.
    vocab = load_heuristics().fundamental_vocabulary
