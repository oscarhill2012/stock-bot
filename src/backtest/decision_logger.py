    ...
    def _build_snapshot(
        ...
        *,
        ...
        return {
            ...
            #
            # ``smart_money`` and ``social`` were removed from this dict because
            # neither has a real data source — both keys were always ``None``,
            # which is misleading "we tried" scaffolding.  To re-add them,
            # uncomment the entries below once the corresponding data provider
            # is wired up and live:
            #
            #   "smart_money": _coerce(
            #       state.get("smart_money_data", {}).get(ticker)
            #   ) if state.get("smart_money_data") else None,
            #   "social": state.get("temp:social_data", {}).get(ticker)
            #             if state.get("temp:social_data") else None,
            "analyst_inputs": {
