"""Loader for ``config/llm_retry.json`` — the retry policy applied to every
...
--------------------
The ``@lru_cache(maxsize=1)`` decorator on :func:`get_retry_config` means
...
``load_retry_config(path=...)`` is the test hook for feeding a custom JSON
...
_DEFAULT_PATH = Path("config/llm_retry.json")


class RetryConfig(BaseModel):
    """Top-level shape of ``config/llm_retry.json``.

    All three fields are bounds on the same exponential-with-jitter
    backoff schedule applied by tenacity inside
    :class:`agents.llm_retry.RetryingAgentWrapper`.
    ...
    max_delay_seconds:
        ...
        ``>= base_delay_seconds``.  Tenacity's ``wait_exponential_jitter``
        will saturate at this value once the exponential growth exceeds
        it.
    """
...
def load_retry_config(*, path: Path | None = None) -> RetryConfig:
    """Read and validate ``config/llm_retry.json``.
    ...
    cfg = RetryConfig.model_validate(payload)
...
def get_retry_config() -> RetryConfig:
    ...
    editing ``config/llm_retry.json`` — there is no hot-reload path.
