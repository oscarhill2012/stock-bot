...
``yfinance`` shell (``options/yfinance.py``).  It returns an empty
``list[OptionContract]`` for any historical ``as_of`` so backtest replay is
never blocked by a missing options cache entry.  A full PIT-correct implementation is deferred to a
