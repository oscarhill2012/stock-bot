"""yfinance provider — ``price_history`` domain only.

The ``company_ratios`` registration was removed in the plan-08 provider cull
(A-038); ``pit_composite`` is now the sole registered ``company_ratios``
provider.

The underlying yfinance call is shared per-ticker per-tick by an in-memory
LRU cache keyed on ``(symbol, period, interval)`` so that requesting
``price_history`` for multiple tickers in one tick does not double the
