...
| `news.llm.max_output_tokens` | int [256–32768] | Cap on output tokens per call. Range `[256, 32768]`. Default 4000 — `gemini-2.5-flash` is a thinking model whose dynamic reasoning tokens are charged against this same budget (observed ~1.6k for thinking alone), so the cap must hold both the thinking and the verdict JSON. Sized up from 2000 (the `gemini-2.5-flash-lite` value) after `flash` thinking starved the JSON and truncated it. |
| `news.llm.thinking_budget` | int [-1–24576] or omitted | Optional ceiling on the model's internal *thinking* tokens (Gemini 2.5 thinking models). On `gemini-2.5-flash` the reasoning tokens are charged against `max_output_tokens`, so leaving thinking *dynamic* (unbounded) lets it spike and truncate the verdict JSON. Currently **512** — bounds thinking so the split with the visible output is deterministic; aligned to the best-run news budget (lower than the fundamental analyst's 2048, since the news verdict is a lighter reasoning task). Omit the field for native dynamic thinking, `0` to disable thinking, `-1` for explicit dynamic. Gemini-specific; a Claude-on-Vertex model ignores it. |
...
| `fundamental.llm.max_output_tokens` | int [256–32768] | Cap on output tokens per call. Range `[256, 32768]`. Default 4000 — `gemini-2.5-flash` is a thinking model whose dynamic reasoning tokens are charged against this same budget (observed ~1.6k for thinking alone), so the cap must hold both the thinking and the verdict JSON. Sized up from 2000 (the `gemini-2.5-flash-lite` value) after `flash` thinking starved the JSON and truncated it. |
| `fundamental.llm.thinking_budget` | int [-1–24576] or omitted | Optional ceiling on the model's internal *thinking* tokens (Gemini 2.5 thinking models). On `gemini-2.5-flash` the reasoning tokens are charged against `max_output_tokens`, so leaving thinking *dynamic* (unbounded) lets it spike and truncate the verdict JSON. Default **2048** — bounds thinking so the split with the visible output is deterministic (2048 thinking + ~1950 JSON inside the 4000 cap). Omit the field for native dynamic thinking, `0` to disable thinking, `-1` for explicit dynamic. Gemini-specific; a Claude-on-Vertex model ignores it. |
