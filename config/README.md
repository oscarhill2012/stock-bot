...
| `fundamental.filing_delta_horizon_days` | int [1–250] | Trading-day horizon injected deterministically from config as the analyst's canonical `horizon_days` at the joiner (the LLM no longer emits it). The filing-delta drift (Cohen, Malloy & Nguyen 2020) operates over 3–6 months; default **60**. |
